...
import { getAllProjectTags, createProjectTag, updateProjectTag, deleteProjectTag } from '../../services/db';
...
export function ProjectTagsPage() {
  const [items, setItems] = useState<ProjectTag[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [deleteModal, setDeleteModal] = useState<ProjectTag | null>(null);
  ...
  const loadItems = async () => {
    try {
      const all = await getAllProjectTags();
  ...
  const openEditModal = (item: ProjectTag) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteProjectTag(deleteModal.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Projekt címkék</h1>
          <p className="text-sm text-gray-500 mt-1">{items.length} címke</p>
        </div>
              ...
              {filteredItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setDeleteModal(item)}>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Címke szerkesztése' : 'Új címke'} size="sm">
      ...
      <Modal isOpen={!!deleteModal} onClose={() => setDeleteModal(null)} title="Címke törlése" size="sm">
        <p className="text-gray-600 mb-6">Biztosan törli a <strong>{deleteModal?.name}</strong> címkét?</p>
